...
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)/../codex"
...
# review-2026-09: reasoning effort pinned explicitly (E5 is a low-effort
# comparison) instead of inherited from this machine's global config.
echo "codex=$(codex --version 2>&1) model=$MODEL effort=${CODEX_EFFORT:-low} startedAt=$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$STATE_DIR/run-meta.txt"
shasum -a 256 kickoff-prompt.txt >> "$STATE_DIR/run-meta.txt"
codex exec \
  --sandbox workspace-write \
  --dangerously-bypass-hook-trust \
  --model "$MODEL" \
  -c model_reasoning_effort="${CODEX_EFFORT:-low}" \
  "$PROMPT" \
