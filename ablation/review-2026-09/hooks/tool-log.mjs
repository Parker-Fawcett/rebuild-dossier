...
process.stdin.on('end', () => {
  try {
    ...
    // Review-2026-09 change (the only one): the single `enforce` marker is
    // split so E4 can lock spec/ and block untested contracts independently.
    // `enforce` alone still means both, so this script behaves byte-for-byte
    // like ../../claude-code/hooks/tool-log.mjs for every older rep layout.
    const enforceAll = existsSync(join(stateDir, 'enforce'));
    const enforceSpec = enforceAll || existsSync(join(stateDir, 'enforce-spec'));
    const enforceUntested = enforceAll || existsSync(join(stateDir, 'enforce-untested'));
    const enforce = enforceSpec || enforceUntested;
