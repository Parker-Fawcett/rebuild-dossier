...
- **2026-09-23 05:50Z.** `B-rep2` (7/12) and `B-rep3` (8/12) completed cleanly. The Haiku batch is 9/20 (A 1, B 4, C 2, D 2) and is paused again. It is still reported descriptively.
- **2026-09-23 ~06:00Z. §11 follow-up prepared (triggered by the E5 verdict).**
  - Reps: `rev-xhigh-rep1/2`, `rev-ultra-rep1/2`, `orig-ultra-rep1/2`, set up by `setup-effort.sh` exactly as every E5 rep.
...
- **2026-09-23 ~13:05Z. On the user's instruction, all remaining reps are launched via `full-chain.sh`, strictly sequential.** Stage 1 is E5: the 5 `gpt-5.5` reps, then the six §11 effort reps. Stage 2 waits for the `claude` CLI's login, which had expired again (confirmed by a one-line probe), then runs the 11 remaining Haiku reps in `run-order.txt` order, then the 10 Sonnet reps (§10). With all of them, the Haiku batch reaches its registered 20 and the §5 decision rules apply as written. Codex availability was confirmed first with a one-line `codex exec` probe, so the parked `g55orig-rep3` re-run is not spent on a known-exhausted quota.
- **2026-09-23 ~13:10Z. At the user's request, the Claude Code batches now run in parallel with Codex, not after it.** This deviates from §1's one-at-a-time rule, which is kept *within* each batch: Haiku reps run sequentially, then Sonnet, and Codex reps run sequentially. The two batches overlap only with each other, in separate directories, CLIs, and sandboxes. Consequence, disclosed: the remaining Haiku and Sonnet reps run while a Codex rep may be using CPU, and the 9 earlier Haiku reps did not. Agents are API-bound, and the local work is short vitest runs, so a behavioral effect is unlikely, but it is not controlled. The `full-chain.sh` controller was stopped (its running `gpt-5.5` batch left intact) and replaced by `codex-tail-chain.sh` (waits on that batch's PID, then runs the §11 follow-up and rescoring) and `claude-chain.sh` (Haiku, then Sonnet).
- **2026-09-23 13:17Z. The user asked to stop after the next rep.** Both chain controllers and loops were stopped, and the two in-flight trials were left to finish. `g55orig-rep2` completed cleanly: all 6 `gpt-5.5` reps are done, with `orig` 0/3 stalled and `rev` 0/3 stalled, all 20/20. §6 reading: the failure is Astra-specific. The Haiku rep in flight (`B-rep4`) is finishing. The §11 effort follow-up and the Sonnet arm (§10) did not start. Everything unrun stays prepared.
- **2026-09-23 13:18Z. Haiku batch stopped at 15/20** (A 2, B 5, C 4, D 4) after the in-flight `B-rep4`. The unrun reps are `A-rep3`, `A-rep1`, `D-rep2`, `A-rep4`, `C-rep2`, still prepared. Only arm B has reached its registered 5, so no §5 comparison is applied. The Sonnet arm (§10) never started.
- **2026-09-23 ~13:30Z. On the user's instruction, all remaining registered reps are launched.**
  - Claude, via `claude-chain.sh`: the 5 remaining Haiku reps in `run-order.txt` order, then the 10 Sonnet reps (§10).
  - Codex, via `codex-effort-chain.sh`: the six §11 reps, then rescoring.
...
- **2026-09-24 00:23Z. §11 follow-up complete, 6/6.** `rev-ultra-rep1` (its one re-run, after the usage-limit park) and `rev-ultra-rep2` both ran at confirmed `ultra` effort: no stall, 16 route files, 20/20 visible, 0 rail attempts. Totals: `rev` stalled 0 of 4 higher-effort reps (`xhigh` 0/2, `ultra` 0/2, all 20/20); `orig`-`ultra` stalled 2/2. **§11 reading, as registered: the fix generalizes across effort.** Scored by `rescore-astra.mjs`; results in `results/E5-astra-results-rescored.json`. Every registered rep in this pre-registration has now run. `results/haiku-sealed-results.json` and `results/sonnet-sealed-results.json` are copies of the final per-arm aggregates.

## 12. E7, external validators (registered 2026-09-24 ~01:30Z, before any validator has started)

Two non-author operators each run `docs/validators.md` (tool pinned at `rebuild-dossier@0.2.10`) once, on an app of their own the author has never worked on. The author does not operate, steer, or debug any run. Questions answered during a run are logged with the report.
- **Endpoints, per report, each reported separately and never merged into one success label:** (1) install-to-package completion (did `generate_spec` produce a package, and every error hit on the way); (2) visible pass/total and held-out pass/total, both over the fixed file denominator, run by the validator after sealing; (3) hook heartbeat present and count; (4) process observations (over-building, test or spec edits, false success claims, attempts to reach the source or the sealed tests); (5) behavioral differences the validator found against the real app, especially ones the tests passed.
- **Reporting rule:** every report is reported, including a run that fails at install or generation, which is itself a result on endpoint (1). A run that departs from the protocol (not sealed, not top-level, a different version) is reported under that label, never excluded. Two runs support existence claims only, not a rate.
- **What changes the paper:** a validator result is added to the manuscript as a new, labeled external observation. If it contradicts a claim the paper makes (for example, a tool bug that invalidates a reported number), the claim is corrected, and a new paper tag and DOI are cut. Results that fit the existing claims are added without re-framing them as confirmation of anything beyond what they measure.
- **2026-09-24 ~05:20Z, deviation before any validator started: the E7 pin moves from 0.2.10 to 0.2.11.** A cold run of `docs/validators.md` by the author found that under `npx`, 0.2.9–0.2.10 cannot run the mutation check at all (vitest is not installed), so every generated test comes back `unrunnable`. A validator on 0.2.10 would have measured that packaging bug, not the tool. No validator had begun; the endpoints and reporting rule above are unchanged.
- **2026-09-24 ~14:00Z, second deviation before any validator started: the E7 pin moves from 0.2.11 to 0.2.12.** A second author cold run, this time on an unfamiliar Express app, found:
  - Apps exported with `module.exports = app` got API tests that could never pass: the generated import bound `undefined`.
  - A todo app's section comments were raised as TODO cases, each filed under the wrong route.
