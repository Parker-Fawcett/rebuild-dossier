...
describe('generateSettingsJson', () => {
  ...
  it('routes every write-capable tool through the generated guard script', () => {
    // Behavior of the guard itself is covered in generateGuardHook.spec.ts,
    // which runs it through a real shell; this only checks the wiring.
    const preToolUse = generateSettingsJson('npm test').hooks.PreToolUse;
